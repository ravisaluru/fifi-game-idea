...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  Future<void> _createRoom() async {
    ...
    try {
      ...
      final code = await MultiplayerService.instance.createRoom(player, worldId);
      setState(() { _myRoomCode = code; _loading = false; });
      _watchRoom(code);
    } catch (e) {
          ...
          child: !widget.firebaseAvailable
            ...
            child: switch (_mode) {
              _LobbyMode.choose => _ChooseView(
                  onCreate: () => setState(() => _mode = _LobbyMode.create),
              ...
              _LobbyMode.create => _myRoomCode == null
                  ? _CreateView(
                      ...
                      onBack: () => setState(() => _mode = _LobbyMode.choose),
                    )
                  : _WaitingRoom(
                      code: _myRoomCode!,
...
class _ChooseView extends StatelessWidget {
  ...
  const _ChooseView({required this.onCreate, required this.onJoin, required this.onBack});
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      _LobbyButton(label: 'Create Room 🏠', onTap: onCreate,
          color: const Color(0xFF1565C0)),
      const SizedBox(height: 16),
      _LobbyButton(label: 'Join Room 🚪', onTap: onJoin,
...
class _CreateView extends StatelessWidget {
  ...
  const _CreateView({required this.loading, required this.error,
      required this.onCreate, required this.onBack});

  @override
  Widget build(BuildContext context) => Column(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      const Text('Create a Room', style: TextStyle(color: Colors.white,
      ...
      if (!loading) _LobbyButton(label: 'Generate Code 🎲',
          onTap: onCreate, color: const Color(0xFF1565C0)),
      const SizedBox(height: 16),
      TextButton(onPressed: onBack,
...
class _JoinView extends StatelessWidget {
  ...
  const _JoinView({required this.controller, required this.loading,
      required this.error, required this.onJoin, required this.onBack});
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        const Text('Join a Room', style: TextStyle(color: Colors.white,
            fontSize: 28, fontWeight: FontWeight.bold)),
        ...
        if (!loading) _LobbyButton(label: 'Join! 🚀', onTap: onJoin,
            color: const Color(0xFF6A1B9A)),
        const SizedBox(height: 16),
        TextButton(onPressed: onBack,
            child: const Text('← Back', style: TextStyle(color: Colors.white70))),
...
class _WaitingRoom extends StatelessWidget {
  ...
  const _WaitingRoom({
    required this.code, required this.players, required this.isHost,
    this.onStart, required this.onLeave,
  });
  ...
  Widget build(BuildContext context) => Column(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      const Text('Waiting Room', style: TextStyle(color: Colors.white,
          fontSize: 28, fontWeight: FontWeight.bold)),
      const SizedBox(height: 24),
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              const Text('Room Code', style: TextStyle(color: Colors.white60,
                  fontSize: 13)),
              const SizedBox(height: 4),
              Text(code, style: const TextStyle(color: Colors.white,
                  fontSize: 48, fontWeight: FontWeight.bold, letterSpacing: 12)),
              const Text('Tap to copy', style: TextStyle(color: Colors.white38,
                  fontSize: 11)),
            ],
          ),
        ),
...
class _LobbyButton extends StatelessWidget {
  ...
  const _LobbyButton({required this.label, required this.color, required this.onTap});
